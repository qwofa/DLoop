...
FRICTION_INBOX = Path(".scratch") / "dloop-v3" / "v4.0.1" / "friction-inbox"
...
class InstallationTests(unittest.TestCase):
    ...
    def test_complete_uninstall_is_safe_and_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            installed = self._run(target, "-Version", f"v{VERSION}")
            self.assertEqual(0, installed.returncode, installed.stderr)
            archive = target / ".scratch" / "dloop-v3" / "v4.0.1" / "outputs" / "kept.json"
            archive.parent.mkdir(parents=True)
            archive.write_text('{"kept": true}\n', encoding="utf-8")
            history = target / ".scratch/dloop-v3/v4.0.1/snapshots/kept.git/objects/retained"
    ...
    def test_installed_runtime_avoids_bytecode_cache_before_uninstall(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            friction = target / ".scratch" / "dloop-v3" / "v4.0.1" / "friction.jsonl"
