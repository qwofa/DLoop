---
name: dloop
description: 使用 DLoop v3.9.4 管理至少包含一个实施切片的大型功能交付流；普通调用不生成 Unity UI 专属产物。
disable-model-invocation: true
---

# DLoop v3.9.4 功能交付流入口
...
交付档案固定在 `.scratch/dloop-v3/v3.9.4/outputs/<feature-id>/`，阶段依次是需求、调查、设计、计划、实施、验证。只通过 `python Tools/FeatureArchive/feature_archive.py <command>` 调用确定性能力；命令不接收路径根，写入使用当次返回的 `archive_location` 和 `write_targets`。
...
执行命令使用工具返回的 `next_action_contract`、`handoff_preparation` 和 `input_preparations` 的完整参数；任务包与执行身份不要混填，下表规则动作不是交接阶段。设计评审交接用 `--action design --role design-review`，候选评审用 `--action implementation --role review --execution-id <评审身份>`。评审分别使用 `review-issues`、`review-verification`。正式输入只用准备入口返回的路径；临时分析用 `.scratch/outputs/`。
