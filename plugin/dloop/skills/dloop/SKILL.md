---
name: dloop
description: 使用 DLoop v3.9.0 管理至少包含一个实施切片的大型功能交付流；普通调用不生成 Unity UI 专属产物。
---

# DLoop v3.9.0 功能交付流入口
...
交付档案固定在 `.scratch/dloop-v3/v3.9.0/outputs/<feature-id>/`，阶段依次是需求、调查、设计、计划、实施、验证。只通过 `python Tools/FeatureArchive/feature_archive.py <command>` 调用确定性能力；命令不接收路径根，写入使用当次返回的 `archive_location` 和 `write_targets`。
