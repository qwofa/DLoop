---
name: dloop
description: 使用 DLoop v4.0.0 管理至少包含一个实施切片的大型功能交付流；普通调用不生成 Unity UI 专属产物。
disable-model-invocation: true
---

# DLoop v4.0.0 功能交付流入口
...
交付档案固定在 `.scratch/dloop-v3/v4.0.0/outputs/<feature-id>/`，阶段依次是需求、调查、设计、计划、实施、验证。已连接 DLoop MCP 时，先核对返回的项目根；状态用 `dloop_status`，四类输入用 `dloop_prepare_input` 取模板后调用对应提交工具，勿手写 JSON 文件。其余动作或未连接时用 `python Tools/FeatureArchive/feature_archive.py <command>`；写入位置以返回为准。
