...
def require_root_contract(root: Path) -> Mapping[str, object]:
    ...
    try:
        ...
        raise ArchiveRootError(
            ...
            f"档案根“{root}”缺少 DLoop 3.0 根清单；历史档案不会被自动读取或迁移。",
        ) from exception
    except (OSError, UnicodeError, json.JSONDecodeError) as exception:
        raise ArchiveRootError(
            "INVALID_ARCHIVE_ROOT",
            f"DLoop 3.0 根清单“{path}”无法读取：{exception}",
        ) from exception
    if value != root_manifest():
        raise ArchiveRootError(
            "INCOMPATIBLE_ARCHIVE_ROOT",
            f"档案根“{root}”不属于 DLoop 3.0 严格合同；历史档案不会被自动读取或迁移。",
        )
    return value


def ensure_root_contract(root: Path) -> bool:
    path = root / ROOT_MANIFEST_NAME
    if path.exists():
    ...
    if any(root.iterdir()):
        raise ArchiveRootError(
            "INCOMPATIBLE_ARCHIVE_ROOT",
            f"档案根“{root}”包含未版本化内容；请为 DLoop 3.0 使用独立空目录。",
        )
    path.write_text(
